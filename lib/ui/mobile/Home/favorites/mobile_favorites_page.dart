...
import 'package:mg_music/ui/mobile/Home/favorites/components/mobile_favorites_shimmer.dart';
...
class MobileFavoritesPageState extends State<MobileFavoritesPage>
  ...
  void _processAndSetFavorites(
    ...
    // Ordenar: 1. Favorito principal, 2. Ado, 3. Título
    favorites.sort((a, b) {
  ...
  Future<void> _setMainFavorite(String songId) async {
    ...
    setState(() {
      ...
      // Re-ordenar la lista inmediatamente para feedback visual
      _favoriteSongs.sort((a, b) {
        final aId = a.id.toString();
        final bId = b.id.toString();
        if (aId == songId) return -1;
        if (bId == songId) return 1;
        final aIsAdo = AdoHandler.isAdo(a);
        final bIsAdo = AdoHandler.isAdo(b);
        if (aIsAdo && !bIsAdo) return -1;
        if (!aIsAdo && bIsAdo) return 1;
